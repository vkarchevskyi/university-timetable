...
@section('title', 'Create Exception')
@section('page-title', 'Create Exception')
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-600 mt-1">Add a new schedule exception to the system.</p>
        </div>

        <form method="POST" action="{{ route('admin.exceptions.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="order" id="order" required
                    ...
                    <option value="1" {{ old('order') == '1' ? 'selected' : '' }}>1st Lesson</option>
                    <option value="2" {{ old('order') == '2' ? 'selected' : '' }}>2nd Lesson</option>
                    <option value="3" {{ old('order') == '3' ? 'selected' : '' }}>3rd Lesson</option>
                    <option value="4" {{ old('order') == '4' ? 'selected' : '' }}>4th Lesson</option>
                    <option value="5" {{ old('order') == '5' ? 'selected' : '' }}>5th Lesson</option>
                </select>
                @error('order')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
